...
func mapPerson(c Contact) people.Person {
	...
	// Append work info to OtherNotes since no start date available from Monica.
...
func mapReminders(mrs []MReminder) []reminders.Reminder {
	out := make([]reminders.Reminder, 0, len(mrs))
	for _, r := range mrs {
