...
function SectionCard({ children }: { children: React.ReactNode }) {
	return <div className="rounded-lg border border-zinc-200 bg-white p-4">{children}</div>
}
...
function OverviewSection({ person, editing, onEdit, onCancel }: OverviewSectionProps) {
	...
	return (
		<div className="space-y-3">
			...
			<AvatarUploader personId={person.id} hasAvatar={Boolean(person.avatar_path)} showControls={editing} />
			...
			{editing ? (
				<div className="space-y-3">
					...
					{!person.is_self && (
						<div>
							...
							<Input value={relationshipType} onChange={(e) => setRelationshipType(e.target.value)} placeholder="e.g. Friend, Colleague" />
						</div>
					)}
					<div>
				...
				<>
					<dl className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
						...
						{person.is_self ? (
							<><dt className="font-medium text-zinc-500">Relationship</dt><dd><Badge variant="neutral">Self profile</Badge></dd></>
...
export function PersonDetailSections({ personId, onClose }: PersonDetailSectionsProps) {
	...
	return (
		<div className="space-y-3">
			...
			<SectionCard><ContactsSection person={person} /></SectionCard>
			<SectionCard><LocationsSection person={person} /></SectionCard>
			<SectionCard><LabelsSection person={person} /></SectionCard>
			<SectionCard><RelationshipsSection personId={personId} /></SectionCard>
			<SectionCard><JournalSection personId={personId} /></SectionCard>
			<SectionCard><WorkHistorySection personId={personId} /></SectionCard>
			<SectionCard><ImportantDatesSection personId={personId} person={person} /></SectionCard>
			<SectionCard><GiftsSection personId={personId} /></SectionCard>
		</div>
	)
}
