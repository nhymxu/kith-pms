...
import type { Person, ContactInfo } from "#/schemas/person"
...
function ContactEditRow({ contact, onSave, onCancel }: EditRowProps) {
	...
	return (
		<div className="flex flex-wrap gap-2 items-center border border-zinc-300 rounded-md p-2 bg-zinc-50">
			...
			<Input className="h-8 flex-1 min-w-[120px]" value={value} onChange={(e) => setValue(e.target.value)} placeholder="Value" />
			<Input className="h-8 w-24" value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Label" />
			<button type="button" onClick={() => onSave({ ...contact, type, value, label })} className="text-green-600 hover:text-green-700">
...
export function ContactsSection({ person }: ContactsSectionProps) {
	...
	const saveMutation = useMutation({
		mutationFn: (contacts: ContactInfo[]) =>
			updatePerson(person.id, {
				...
				contacts: contacts.map((c, i) => ({ type: c.type, value: c.value, label: c.label, position: i })),
				locations: person.locations.map((l, i) => ({
					type: l.type, address: l.address, city: l.city,
					country: l.country, postal_code: l.postal_code, position: i,
				})),
			}),
		onSuccess: () => {
	...
	function handleSave(updated: Partial<ContactInfo>) {
		...
		if (editingId === "new") {
			contacts = [...person.contacts, { id: 0, person_id: person.id, type: updated.type!, value: updated.value!, label: updated.label ?? "", position: person.contacts.length }]
		} else {
			contacts = person.contacts.map((c) => c.id === editingId ? { ...c, ...updated } : c)
		}
	...
	function handleDelete(id: number) {
		...
		saveMutation.mutate(contacts, { onSuccess: () => setConfirmContactId(null) })
	}
		...
		<div>
			...
			<div className="space-y-2">
				{person.contacts.map((c) =>
					editingId === c.id ? (
						<ContactEditRow key={c.id} contact={c} onSave={handleSave} onCancel={() => setEditingId(null)} />
					) : (
						<div key={c.id} className="flex gap-3 text-sm border border-zinc-200 rounded-md p-2 items-center">
							...
							<button type="button" onClick={() => setEditingId(c.id)} className="text-foreground/40 hover:text-main">
								<Pencil className="size-3" />
							</button>
							<button type="button" onClick={() => setConfirmContactId(c.id)} className="text-foreground/40 hover:text-destructive">
				...
				{editingId === "new" && (
					<ContactEditRow contact={{}} onSave={handleSave} onCancel={() => setEditingId(null)} />
				)}
				{person.contacts.length === 0 && editingId !== "new" && (
					<p className="text-sm text-zinc-400">No contacts.</p>
				)}
			</div>
			<Dialog open={confirmContactId !== null} onOpenChange={(v) => !v && setConfirmContactId(null)}>
				<DialogContent>
					...
					{(() => {
						...
						return c ? (
							<p className="text-[13px] text-zinc-600">
								Remove the <span className="font-medium">{c.type}</span> contact <span className="font-medium">{c.value}</span>{c.label ? ` (${c.label})` : ""}?
							</p>
						) : null
					})()}
					<DialogFooter>
						<Button variant="neutral" onClick={() => setConfirmContactId(null)}>Cancel</Button>
						<Button
							variant="destructive"
							disabled={saveMutation.isPending}
							onClick={() => confirmContactId !== null && handleDelete(confirmContactId)}
						>
