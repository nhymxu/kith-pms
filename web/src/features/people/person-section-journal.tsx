		...
		<div>
			...
			{entries.length === 0 ? (
				...
				<div className="space-y-2">
					{entries.map((e) => (
						<Link key={e.id} to="/journal/$entryId" params={{ entryId: String(e.id) }} className="block p-2 border border-zinc-200 rounded-md hover:bg-zinc-50 text-sm">
							...
							{e.people.length > 1 && (
								<div className="flex gap-1 mt-1 flex-wrap">
									{e.people.filter((p) => p.person_id !== personId).map((p) => (
										<span key={p.person_id} className="text-[11px] text-zinc-400">{p.name}</span>
									))}
								</div>
							)}
						</Link>
