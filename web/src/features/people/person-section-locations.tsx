...
import type { Person, Location } from "#/schemas/person"
		...
		<div className="border border-zinc-300 rounded-md p-3 bg-zinc-50 space-y-2">
			<div className="flex gap-2 items-center">
				...
				<div className="flex gap-1 ml-auto">
					<button type="button" onClick={() => onSave({ ...location, type, address, city, country, postal_code: postalCode })} className="text-green-600 hover:text-green-700">
			...
			<Input className="h-8" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" />
			<div className="flex gap-2">
				<Input className="h-8 flex-1" value={city} onChange={(e) => setCity(e.target.value)} placeholder="City" />
				<Input className="h-8 w-24" value={postalCode} onChange={(e) => setPostalCode(e.target.value)} placeholder="Postal code" />
				<Input className="h-8 flex-1" value={country} onChange={(e) => setCountry(e.target.value)} placeholder="Country" />
			</div>
		</div>
	)
}
	...
	const saveMutation = useMutation({
		mutationFn: (locations: Location[]) =>
			updatePerson(person.id, {
				...
				contacts: person.contacts.map((c, i) => ({ type: c.type, value: c.value, label: c.label, position: i })),
	...
	function handleSave(updated: Partial<Location>) {
		...
		if (editingId === "new") {
			locations = [...person.locations, {
				id: 0, person_id: person.id,
				type: updated.type!, address: updated.address ?? "",
				city: updated.city ?? "", country: updated.country ?? "",
				postal_code: updated.postal_code ?? "", position: person.locations.length,
			}]
		} else {
			locations = person.locations.map((l) => l.id === editingId ? { ...l, ...updated } : l)
		}
		...
		<div>
			...
			<div className="space-y-2">
				{person.locations.map((l) =>
					editingId === l.id ? (
						<LocationEditRow key={l.id} location={l} onSave={handleSave} onCancel={() => setEditingId(null)} />
					) : (
						<div key={l.id} className="text-sm border border-zinc-200 rounded-md p-3 space-y-1">
							<div className="flex items-center gap-2">
								<Badge variant="neutral">{l.type}</Badge>
								<div className="flex gap-1 ml-auto">
									<button type="button" onClick={() => setEditingId(l.id)} className="text-foreground/40 hover:text-main">
										<Pencil className="size-3" />
									</button>
									<button type="button" onClick={() => setConfirmDeleteId(l.id)} className="text-foreground/40 hover:text-destructive">
				...
				{editingId === "new" && (
					<LocationEditRow location={{}} onSave={handleSave} onCancel={() => setEditingId(null)} />
				)}
					...
					{(() => {
						...
						return l ? (
							<p className="text-[13px] text-zinc-600">
								Remove the <span className="font-medium">{l.type}</span> location{l.address ? ` at ${l.address}` : ""}?
							</p>
						) : null
					})()}
					<DialogFooter>
